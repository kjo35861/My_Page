(() => {
  ...
  data.works.works.forEach(w=>{
    ...
    const action=w.href?`<a class="button primary status" href="${w.href}" target="_blank" rel="noreferrer">${w.id==='paper10'?'논문 열기':'앱 열기'}</a>`:`<span class="button status" aria-disabled="true">${w.status==='planned'?'과제 13 완료 후 연결':'공개 링크 확인 후 연결'}</span>`;
